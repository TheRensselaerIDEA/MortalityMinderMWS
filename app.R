...
serv_calc[[11]] <- function(calc, session) {
  calc$mort.cluster.raw <- reactive({
    ...
    if (calc$state_choice == "United States"){
      ...
      cluster.counties(cdc.mort.mat(cdc.data, "US", calc$death_cause),
                       cluster.method="kmeans",
                       cluster.num=n.clusters)
    }
    else{
      state.data <- cdc.mort.mat(cdc.data, calc$state_choice, calc$death_cause)
...
serv_calc[[18]] <- function(calc, session) {
  observe({
    ...
    if (is.null(event))
      return()
    highlight_county(event)
    calc$county_choice(paste0(event$id," County"))
...
serv_out[["determinants_plot1"]] <- function(calc, session) {
  renderPlot({
    # Sort by kendall.cor
    ...
    assign("kendall_cor_new", calc$kendall.cor.new, envir = .GlobalEnv)
...
serv_out[["determinants_plot2"]] <- function(calc, session) {
  renderPlot({
    ...
    res <- dplyr::inner_join(calc$mort.cluster.ord(), res, by = 'county_fips')
    ...
    if (nrow(sd.select) == 0) {
      ...
      ggplot(sd.select, aes(x = cluster, y = VAR, fill = cluster)) + 
        geom_boxplot() +
        labs(y = calc$determinant_choice, 
